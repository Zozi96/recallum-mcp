...
Ensure Claude Code—both the terminal CLI and the Claude.app Desktop Code host—can discover and call Recallum MCP tools after a standard install, without depending solely on plugin-bundled `${user_config.*}` expansion that Desktop sessions have been observed not to register into the deferred tool catalog.
